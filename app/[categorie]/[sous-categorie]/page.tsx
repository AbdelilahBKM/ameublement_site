...
const Page = () => {
    ...
    const encodedString = params['sous-categorie'];
    const sous_categorie = decodeURIComponent(encodedString.toString());
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const [SelectedCategorie, setSelectedCategorie] = useState<Categorie | null>(null);
  const [subCategorie, setSubCategorie] = useState<Categorie | null>(null);
  ...
  useEffect(() => {
    const fetchCategorie = async () => {
      ...
      try {
        ...
        const sub = data.children.filter((cat) => cat.nom_categorie.toLowerCase() === sous_categorie.toLowerCase());
        setSubCategorie(sub[0]);
      } catch (error) {
        console.error("Error fetching products:", error);
      }
    };
    fetchCategorie();
  }, [categorie, encodedString]);

  useEffect(() => {
    const fetchProduits = async () => {
      try{
        setIsLoading(true);
        const response = await fetch(`${DOMAIN_NAME}/api/produits/by_sub_category/${subCategorie?.id}`, {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
          },
  ...
  }, [SelectedCategorie, sous_categorie]);
  ...
  return (
    <main className="min-h-screen">
      ...
      { !isLoading && (!SelectedCategorie || !subCategorie) ? <NotFound /> :
        <section className="py-16">
        <div className="container mx-auto max-w-6xl">
          <div className="mb-8 flex justify-center">
            <div className="flex justify-center">
              <h2 className="text-2xl text-center font-bold text-slate-700 md:max-w-[600px] leading-7">{`${SelectedCategorie?.nom_categorie && SelectedCategorie?.nom_categorie} / ${subCategorie?.nom_categorie &&  subCategorie?.nom_categorie} Produits`}</h2>
            </div>
          </div>
